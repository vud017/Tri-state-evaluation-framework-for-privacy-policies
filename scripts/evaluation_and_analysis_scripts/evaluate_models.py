import argparse
import csv
import json
import os
from collections import defaultdict
from typing import Any, Dict, List, Optional, Tuple

"""
Evaluation script for LLM privacy policy annotation.

Compares model-generated JSON annotations against human made annotations
using two metrics as defined in the methodology:
  - Agreement rate: proportion of attributes with identical labels
  - Macro-averaged F1: one-vs-rest F1 per label (true/false/ambiguous), then averaged

Inputs:
  --human_annotation  : directory containing human-annotated JSON files (one per policy)
  --model_dirs    : one or more model result directories (each named after the model)
  --out           : output directory for CSV results

Outputs:
  - results_per_model.csv        : overall metrics per model
  - results_per_model_category.csv : metrics broken down by data category
  - results_per_model_field.csv  : metrics broken down by attribute field
  - results_per_policy.csv       : metrics broken down by policy per model

Usage:
  python evaluate_models.py \
    --human_annotation human_annotations/ \
    --model_dirs results/anthropic__claude-sonnet-4-6 results/openai__gpt-5.2-instant ... \
    --out evaluation_out/
"""


TRI_VALUES = ("true", "false", "ambiguous")

TRI_FIELDS: List[Tuple[str, Tuple[str, ...]]] = [
    ("collected.detected",              ("collected", "detected")),
    ("stored.detected",                 ("stored", "detected")),
    ("shared.detected",                 ("shared", "detected")),
    ("shared.third_country_sharing",    ("shared", "third_country_sharing")),
    ("retention_policy.deletion",       ("retention_policy", "deletion")),
    ("retention_policy.inactivity",     ("retention_policy", "inactivity")),
    ("data_minimization.adequate",      ("data_minimization", "adequate")),
]

DATA_CATEGORIES = [
    "biometric_data",
    "health_data",
    "physiological_data",
    "physical_data",
    "behavioral_data",
]


# =========================
# UTILITY
# =========================

def safe_get(d: Dict[str, Any], path_parts: Tuple[str, ...]) -> Optional[Any]:
    cur: Any = d
    for p in path_parts:
        if not isinstance(cur, dict) or p not in cur:
            return None
        cur = cur[p]
    return cur


def normalize_tri(v: Any) -> Optional[str]:
    if v is None:
        return None
    if isinstance(v, bool):
        return "true" if v else "false"
    if isinstance(v, str):
        s = v.strip().lower()
        if s in TRI_VALUES:
            return s
    return None


def load_json(path: str) -> Dict[str, Any]:
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)


def list_policy_files(directory: str) -> List[str]:
    """List all JSON files in a directory, excluding metadata files."""
    files = []
    for name in sorted(os.listdir(directory)):
        if name.lower().endswith(".json") and not name.lower().endswith("_meta.json"):
            files.append(name)
    return files


def write_csv(path: str, rows: List[Dict[str, Any]], fieldnames: List[str]) -> None:
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames)
        w.writeheader()
        for r in rows:
            w.writerow(r)


# =========================
# METRICS
# =========================

def agreement_rate(pairs: List[Tuple[str, str]]) -> float:
    """Proportion of (model, expert) pairs with identical labels."""
    if not pairs:
        return float("nan")
    return sum(1 for m, e in pairs if m == e) / len(pairs)


def macro_f1(pairs: List[Tuple[str, str]]) -> Tuple[float, Dict[str, float]]:
    """
    Compute macro-averaged F1 using one-vs-rest formulation.
    Returns (macro_f1_score, {label: f1_score}).
    """
    per_label_f1 = {}

    for label in TRI_VALUES:
        tp = sum(1 for m, e in pairs if m == label and e == label)
        fp = sum(1 for m, e in pairs if m == label and e != label)
        fn = sum(1 for m, e in pairs if m != label and e == label)

        precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
        recall    = tp / (tp + fn) if (tp + fn) > 0 else 0.0

        if (precision + recall) > 0:
            f1 = 2 * precision * recall / (precision + recall)
        else:
            f1 = 0.0

        per_label_f1[label] = f1

    macro = sum(per_label_f1.values()) / len(TRI_VALUES)
    return macro, per_label_f1


# =========================
# EXTRACTION
# =========================

def extract_pairs(
    gt_doc: Dict[str, Any],
    model_doc: Dict[str, Any],
    category_filter: Optional[str] = None,
    field_filter: Optional[str] = None,
) -> List[Tuple[str, str]]:
    """
    Extract (model_label, expert_label) pairs from two JSON documents.
    Optionally filter by category and/or field name.
    Only includes pairs where both labels are valid tri-state values.
    """
    pairs = []
    categories = [category_filter] if category_filter else DATA_CATEGORIES

    for cat in categories:
        gt_cat   = (gt_doc.get("data_categories") or {}).get(cat, {})
        mod_cat  = (model_doc.get("data_categories") or {}).get(cat, {})

        fields = [f for f in TRI_FIELDS if f[0] == field_filter] if field_filter else TRI_FIELDS

        for field_name, path_parts in fields:
            gt_val  = normalize_tri(safe_get(gt_cat, path_parts))
            mod_val = normalize_tri(safe_get(mod_cat, path_parts))

            if gt_val in TRI_VALUES and mod_val in TRI_VALUES:
                pairs.append((mod_val, gt_val))

    return pairs


# =========================
# MAIN EVALUATION
# =========================

def evaluate_model(
    model_dir: str,
    gt_dir: str,
) -> Dict[str, Any]:
    """
    Evaluate a single model directory against the "ground truth" directory.
    Returns a nested dict of results keyed by scope.
    """
    model_name = os.path.basename(model_dir)
    gt_files = set(list_policy_files(gt_dir))
    model_files = set(list_policy_files(model_dir))
    common_files = sorted(gt_files & model_files)

    if not common_files:
        print(f"  WARNING: No common policy files found for model '{model_name}'")
        return {}

    print(f"  Evaluating {model_name} on {len(common_files)} policies: {common_files}")

    # Collect all pairs across scopes
    all_pairs:                              List[Tuple[str, str]] = []
    pairs_by_category:  Dict[str, List]    = defaultdict(list)
    pairs_by_field:     Dict[str, List]    = defaultdict(list)
    pairs_by_policy:    Dict[str, List]    = defaultdict(list)

    for fname in common_files:
        policy_id = os.path.splitext(fname)[0]
        gt_doc    = load_json(os.path.join(gt_dir, fname))
        mod_doc   = load_json(os.path.join(model_dir, fname))

        for cat in DATA_CATEGORIES:
            for field_name, path_parts in TRI_FIELDS:
                gt_cat  = (gt_doc.get("data_categories") or {}).get(cat, {})
                mod_cat = (mod_doc.get("data_categories") or {}).get(cat, {})

                gt_val  = normalize_tri(safe_get(gt_cat, path_parts))
                mod_val = normalize_tri(safe_get(mod_cat, path_parts))

                if gt_val in TRI_VALUES and mod_val in TRI_VALUES:
                    pair = (mod_val, gt_val)
                    all_pairs.append(pair)
                    pairs_by_category[cat].append(pair)
                    pairs_by_field[field_name].append(pair)
                    pairs_by_policy[policy_id].append(pair)

    # Compute overall metrics
    macro, per_label = macro_f1(all_pairs)
    agr = agreement_rate(all_pairs)

    result = {
        "model":             model_name,
        "n_policies":        len(common_files),
        "n_pairs":           len(all_pairs),
        "agreement_rate":    agr,
        "macro_f1":          macro,
        "f1_true":           per_label["true"],
        "f1_false":          per_label["false"],
        "f1_ambiguous":      per_label["ambiguous"],
        "by_category":       {},
        "by_field":          {},
        "by_policy":         {},
    }

    # By category
    for cat, pairs in pairs_by_category.items():
        m, pl = macro_f1(pairs)
        result["by_category"][cat] = {
            "n_pairs":        len(pairs),
            "agreement_rate": agreement_rate(pairs),
            "macro_f1":       m,
            "f1_true":        pl["true"],
            "f1_false":       pl["false"],
            "f1_ambiguous":   pl["ambiguous"],
        }

    # By field
    for fld, pairs in pairs_by_field.items():
        m, pl = macro_f1(pairs)
        result["by_field"][fld] = {
            "n_pairs":        len(pairs),
            "agreement_rate": agreement_rate(pairs),
            "macro_f1":       m,
            "f1_true":        pl["true"],
            "f1_false":       pl["false"],
            "f1_ambiguous":   pl["ambiguous"],
        }

    # By policy
    for pol, pairs in pairs_by_policy.items():
        m, pl = macro_f1(pairs)
        result["by_policy"][pol] = {
            "n_pairs":        len(pairs),
            "agreement_rate": agreement_rate(pairs),
            "macro_f1":       m,
            "f1_true":        pl["true"],
            "f1_false":       pl["false"],
            "f1_ambiguous":   pl["ambiguous"],
        }

    return result


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--ground_truth", required=True,
                    help="Directory containing ground truth JSON files.")
    ap.add_argument("--model_dirs", nargs="+", required=True,
                    help="One or more model result directories.")
    ap.add_argument("--out", required=True,
                    help="Output directory for CSV results.")
    args = ap.parse_args()

    os.makedirs(args.out, exist_ok=True)

    all_results = []
    for model_dir in args.model_dirs:
        print(f"\nEvaluating: {model_dir}")
        res = evaluate_model(model_dir, args.ground_truth)
        if res:
            all_results.append(res)

    if not all_results:
        raise SystemExit("No results to write. Check that model and ground truth directories share policy filenames.")

    # --- Output 1: Overall per model ---
    overall_rows = []
    for r in all_results:
        overall_rows.append({
            "model":          r["model"],
            "n_policies":     r["n_policies"],
            "n_pairs":        r["n_pairs"],
            "agreement_rate": round(r["agreement_rate"], 4),
            "macro_f1":       round(r["macro_f1"], 4),
            "f1_true":        round(r["f1_true"], 4),
            "f1_false":       round(r["f1_false"], 4),
            "f1_ambiguous":   round(r["f1_ambiguous"], 4),
        })
    write_csv(
        os.path.join(args.out, "results_per_model.csv"),
        overall_rows,
        ["model", "n_policies", "n_pairs", "agreement_rate", "macro_f1",
         "f1_true", "f1_false", "f1_ambiguous"],
    )

    # --- Output 2: Per model per category ---
    category_rows = []
    for r in all_results:
        for cat, metrics in r["by_category"].items():
            category_rows.append({
                "model":          r["model"],
                "category":       cat,
                "n_pairs":        metrics["n_pairs"],
                "agreement_rate": round(metrics["agreement_rate"], 4),
                "macro_f1":       round(metrics["macro_f1"], 4),
                "f1_true":        round(metrics["f1_true"], 4),
                "f1_false":       round(metrics["f1_false"], 4),
                "f1_ambiguous":   round(metrics["f1_ambiguous"], 4),
            })
    write_csv(
        os.path.join(args.out, "results_per_model_category.csv"),
        category_rows,
        ["model", "category", "n_pairs", "agreement_rate", "macro_f1",
         "f1_true", "f1_false", "f1_ambiguous"],
    )

    # --- Output 3: Per model per field ---
    field_rows = []
    for r in all_results:
        for fld, metrics in r["by_field"].items():
            field_rows.append({
                "model":          r["model"],
                "field":          fld,
                "n_pairs":        metrics["n_pairs"],
                "agreement_rate": round(metrics["agreement_rate"], 4),
                "macro_f1":       round(metrics["macro_f1"], 4),
                "f1_true":        round(metrics["f1_true"], 4),
                "f1_false":       round(metrics["f1_false"], 4),
                "f1_ambiguous":   round(metrics["f1_ambiguous"], 4),
            })
    write_csv(
        os.path.join(args.out, "results_per_model_field.csv"),
        field_rows,
        ["model", "field", "n_pairs", "agreement_rate", "macro_f1",
         "f1_true", "f1_false", "f1_ambiguous"],
    )

    # --- Output 4: Per model per policy ---
    policy_rows = []
    for r in all_results:
        for pol, metrics in r["by_policy"].items():
            policy_rows.append({
                "model":          r["model"],
                "policy":         pol,
                "n_pairs":        metrics["n_pairs"],
                "agreement_rate": round(metrics["agreement_rate"], 4),
                "macro_f1":       round(metrics["macro_f1"], 4),
                "f1_true":        round(metrics["f1_true"], 4),
                "f1_false":       round(metrics["f1_false"], 4),
                "f1_ambiguous":   round(metrics["f1_ambiguous"], 4),
            })
    write_csv(
        os.path.join(args.out, "results_per_policy.csv"),
        policy_rows,
        ["model", "policy", "n_pairs", "agreement_rate", "macro_f1",
         "f1_true", "f1_false", "f1_ambiguous"],
    )

    # --- Print summary ---
    print("\n===== EVALUATION SUMMARY =====")
    print(f"{'Model':<40} {'Agree':>8} {'MacroF1':>9} {'F1-true':>9} {'F1-false':>9} {'F1-ambig':>10}")
    print("-" * 90)
    for r in overall_rows:
        print(f"{r['model']:<40} {r['agreement_rate']:>8.4f} {r['macro_f1']:>9.4f} "
              f"{r['f1_true']:>9.4f} {r['f1_false']:>9.4f} {r['f1_ambiguous']:>10.4f}")

    print(f"\nOutputs written to: {args.out}")


if __name__ == "__main__":
    main()